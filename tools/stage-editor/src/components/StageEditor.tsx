...
export const StageEditor = ({ stage, onChange, hasStageErrors }: Props) => {
  ...
  return <section className="card">
    ...
    {stage.locations.map((l, i) => <div className="row-inline" key={getRowKey(l, 'location')}><input className="js-location-id" value={l.locationId} onChange={e => update('locations', stage.locations.map((x, n) => n === i ? { ...x, locationId: e.target.value } : x))} placeholder="locationId（例: left）" /><input className="js-location-display-name" value={l.displayName} onChange={e => update('locations', stage.locations.map((x, n) => n === i ? { ...x, displayName: e.target.value } : x))} placeholder="表示名（例: 左岸）" /><button className="js-delete-location button-danger" onClick={() => update('locations', stage.locations.filter((_, x) => x !== i))}>削除</button></div>)}
    ...
    {stage.routes.map((r, i) => <div className="row-inline" key={getRowKey(r, 'route')}><input className="js-route-id" value={r.routeId} onChange={e => update('routes', stage.routes.map((x, n) => n === i ? { ...x, routeId: e.target.value } : x))} placeholder="routeId（例: left_to_right）" /><input className="js-route-from" value={r.from} onChange={e => update('routes', stage.routes.map((x, n) => n === i ? { ...x, from: e.target.value } : x))} placeholder="from locationId" /><input className="js-route-to" value={r.to} onChange={e => update('routes', stage.routes.map((x, n) => n === i ? { ...x, to: e.target.value } : x))} placeholder="to locationId" /><label>双方向<input className="js-route-bidirectional" type="checkbox" checked={r.bidirectional} onChange={e => update('routes', stage.routes.map((x, n) => n === i ? { ...x, bidirectional: e.target.checked } : x))} /></label><button className="js-delete-route button-danger" onClick={() => update('routes', stage.routes.filter((_, x) => x !== i))}>削除</button></div>)}
    ...
    {stage.entities.map((r, i) => <div className="row-inline" key={getRowKey(r, 'entity')}><input className="js-entity-id" value={r.entityId} onChange={e => update('entities', stage.entities.map((x, n) => n === i ? { ...x, entityId: e.target.value } : x))} placeholder="entityId（例: wolf）" /><input className="js-entity-display-name" value={r.displayName} onChange={e => update('entities', stage.entities.map((x, n) => n === i ? { ...x, displayName: e.target.value } : x))} placeholder="表示名（例: オオカミ）" /><input className="js-entity-start-location" value={r.startLocation} onChange={e => update('entities', stage.entities.map((x, n) => n === i ? { ...x, startLocation: e.target.value } : x))} placeholder="startLocation（例: left）" /><input className="js-entity-sprite-id" value={r.spriteId} onChange={e => update('entities', stage.entities.map((x, n) => n === i ? { ...x, spriteId: e.target.value } : x))} placeholder="spriteId" /><label>操船<input className="js-entity-can-operate-boat" type="checkbox" checked={r.canOperateBoat} onChange={e => update('entities', stage.entities.map((x, n) => n === i ? { ...x, canOperateBoat: e.target.checked } : x))} /></label><button className="js-delete-entity button-danger" onClick={() => update('entities', stage.entities.filter((_, x) => x !== i))}>削除</button></div>)}
    ...
    {stage.clearConditions.map((c, i) => <div className="row-inline" key={getRowKey(c, 'clear-condition')}><input className="js-clear-condition-type" value={c.conditionType} onChange={e => update('clearConditions', stage.clearConditions.map((x, n) => n === i ? { ...x, conditionType: e.target.value } : x))} placeholder="conditionType（例: all_entities_at_location）" /><input className="js-clear-condition-target-location" value={c.targetLocationId} onChange={e => update('clearConditions', stage.clearConditions.map((x, n) => n === i ? { ...x, targetLocationId: e.target.value } : x))} placeholder="targetLocationId（例: right）" /><button className="js-delete-clear-condition button-danger" onClick={() => update('clearConditions', stage.clearConditions.filter((_, x) => x !== i))}>削除</button></div>)}
  </section>;
};
